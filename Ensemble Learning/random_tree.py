...
def get_tree(train_data, label_name, label, pre_feature, node, max_depth, method, feature_keep):
    if train_data.shape[0] > 0 and max_depth > 0:
        best_feature = get_best_feature(train_data, label_name, label, method, feature_keep)
        ...
        for i,type in list(next_node.items()):
            if type == "multi":
                ...
                get_tree(sub_feature_data, label_name, label, i, next_node, max_depth-1, method, feature_keep)

def get_id3_tree(train_data, label_name, label,max_depth, method, feature_keep):
    tree_dict = {}
    get_tree(train_data, label_name, label, None, tree_dict, max_depth, method, feature_keep)
    return tree_dict
...
def get_best_feature(train_data, label_name, label,method, feature_keep):
    feature_drop = [label_name,"pos_count", "neg_count"]
    if len(train_data.columns) > feature_keep + 4:
        features = list(train_data.columns)
        features.remove("class")
        features.remove("pos_count")
        features.remove("neg_count")
        features_to_drop = random.sample(features, len(features) - feature_keep)
        feature_drop += features_to_drop
    #print(feature_drop)
    feature_only = train_data.columns.drop(feature_drop)
    #print(feature_only)
    max_info_gain = -1
    best_feature = None
...
def random_tree(train_data, test_data, label_list):
    ...
    for feature_number in feature_subset:
        errors_of_train = []
        errors_of_test = []
        for i in range(0, 500):
            sample = train_data.sample(frac=.6, replace=True).reset_index(drop=True)
            subsample_tree = get_id3_tree(sample, "class", label_list, 16, "entropy", feature_number)
            train_error = get_bagging_error(subsample_tree, train_data, "class")
            errors_of_train.append(train_error)
            test_error = get_bagging_error(subsample_tree, test_data, "class")
            errors_of_test.append(test_error)
        ...
        test_data["neg_count"] = np.zeros(test_data.shape[0])
        test_data["pos_count"] = np.zeros(test_data.shape[0])
        train_data["neg_count"] = np.zeros(test_data.shape[0])
        train_data["pos_count"] = np.zeros(test_data.shape[0])
        ploting(errors_of_train, errors_of_test, "Train Error", "Test Error",
                "Error With 0-500 randomTrees with feature set " + str(feature_number))

def random_100_tree(train_data, test_data, label_list):
    first_100_tree_prediction = []
    bagging_100_tree_prediction = []
    for i in range(0, 100):
        sub_sample = train_data.sample(frac=.2, replace=False).reset_index(drop=True)
        for j in range(0, 500):
            sample = sub_sample.sample(frac=1, replace=True).reset_index(drop=True)
            subsample_tree = get_id3_tree(sample, "class", label_list, 16, "entropy", 4)
            predict_list = get_bagging_bias_result(subsample_tree, test_data, "class")
            if j == 0:
                first_100_tree_prediction.append(predict_list)
    ...
    single_avg_bias, single_avg_var, single_sq_error = cal_bias_variance(first_100_tree_prediction, test_data["class"])
    print("single tree bias, variance, mean sq error", single_avg_bias, single_avg_var, single_sq_error)
    bagging_avg_bias, bagging_avg_var, bagging_sq_error = cal_bias_variance(bagging_100_tree_prediction,
                                                                            test_data["class"])
    print("random forests bias, variance, mean sq error", bagging_avg_bias, bagging_avg_var, bagging_sq_error)
...
if __name__ == '__main__':
    ...
    random_tree(train_data_bank, test_data_bank, label_bank)
    random_100_tree(train_data_bank, test_data_bank, label_bank)
    """
    for method in method_list:
